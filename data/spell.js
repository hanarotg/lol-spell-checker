let obj_org = {
  ...
  data: {
    ...
    SummonerExhaust: {
      ...
      effectBurn: [null, "3", "35", "0", "0", "30", "0", "0", "0", "0", "0"],
      vars: [],
      key: "3",
      summonerLevel: 4,
      modes: [
    ...
    SummonerSmite: {
      ...
      name: "Smite",
      description:
        "Deals 450 true damage to target epic, large, or medium monster or minion. Against monsters, additionally restores 90 (+10% maximum Health) Health. Holds two charges (90s Recharge Time)",
      tooltip:
        'Deals <span class="colorFEFCFF">{{ smitebasedamage }}</span> true damage to target monster or minion. Against monsters, additionally restores <span class="colorFFFFFF">{{ smitebaseheal }}</span> <span class="colorFF6666">(+{{ tooltipsmitehealaddition }})</span> Health.<br /><br />Smite regains a charge every {{ ammorechargetime }} seconds, up to a maximum of 2 charges.<br /><br /><rules>Cannot be cast on small monsters.</rules>',
      maxrank: 1,
      cooldown: [15],
      cooldownBurn: "15",
      cost: [0],
...
Object.values(obj_org.data).forEach((data) => {
  console.log(data.key, data.name, data.cooldown[0]);

  let obj_tmp = {};

  obj_tmp.name = data.name;
  obj_tmp.cooldown = data.cooldown[0];
  obj[data.key] = obj_tmp;
});
